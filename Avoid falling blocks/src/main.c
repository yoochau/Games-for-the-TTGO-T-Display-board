...
void blocks_game(){
   ...
   for(int i=0;i<total;i++) {
        blocks[i].x=rand()%(display_width-10);
        blocks[i].y=display_height+rand()%(display_height*100);
        blocks[i].speed=1;
        blocks[i].colour=rand();
   }
   while(1) {
       cls(rgbToColour(0,0,100));
       snprintf(buff, 128, "Scores:%d", score);
       setFontColour(0, 0, 0);
       print_xy(buff, 12, 12);
       setFontColour(20, 200, 200);
       print_xy(buff, 10, 10);
       draw_rectangle(x,display_height-h,w,h,0xff00);

       
       for(int i=0;i<total;i++) {
            //if(i==0) start_time=esp_timer_get_time();
            draw_rectangle(blocks[i].x,blocks[i].y,h,w,blocks[i].colour);
            blocks[i].y += blocks[i].speed;

            if(blocks[i].y>=display_height*100) {
                blocks[i].x=rand()%(display_width-10);
                blocks[i].y=0;

                now=esp_timer_get_time();

                blocks[i].speed=1+(score/800); //Dropping faster as time passes.
                /*if(total<125){
                    total=total+(score/800); //More dropping blocks as time passes.
                }*/
            }
            if(blocks[i].y>=display_height-35&&blocks[i].y<=display_height+1){
               if(x<=blocks[i].x-34||x>=blocks[i].x+6){
                  if(blocks[i].y>display_height-1&&blocks[i].y<display_height+1){
                    score=score+100;
                  }
               }else{
                 while(i!=0){
                   snprintf(buff, 128, "Game over.Scores:%d", score);
                   setFontColour(0, 0, 0);
                   print_xy(buff, 12, display_height / 2);
                   setFontColour(20, 200, 200);
                   print_xy(buff, 10, display_height / 2);
                   for(int j=0;j<total;j++){
                       if (blocks[j].y<display_height) blocks[j].y=display_height+rand()%(display_height*180);
                   }
                   sleep(6);
                   ...
                   /*int key1=get_input();
                   if(key1==0){
                   //blocks->y=rand()%(display_height*180);
                     i=0;
                     print_xy("/r ", 10, display_height / 2);
                   }*/
               }  
            }
        }
           ...
           if(key==35) {
             x+=3;
             if(x>display_width-w) x=display_width-w;
           }
   }
...
int demo_menu(int select) {
    ...
    // cube vertices
        ...
        for(int frame=0;frame < 4000; frame++) {
            ...
            print_xy("Avoid blocks Game", 10, 10);
            print_xy("Press left or 1 to begin",10,LASTY+24);
            print_xy("In the Game, ",10,LASTY+24);
            print_xy("Press L or 1 to Move Left",10,LASTY+24);
            print_xy("Press R or 2 to Move Right",10,LASTY+24);
            ...
            int key=get_input();
            if(key==0) return select;
            if(key==35) select=(select+1)%4;
        }
    }
}
